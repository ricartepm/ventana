<div id="header">
 ...
 <?php if (isset ($_POST['enviar1'])){
	...
	$qrUpdateRead = "UPDATE solicitacao SET status = '1', data_trans = NOW() WHERE nivel = '$nivel' AND id_solicitacao = '$soli'";
  ...
  <?php if (isset ($_POST['enviar2'])){
	...
	$qrUpdateRead = "UPDATE solicitacao SET status = '2', data_devo = NOW() WHERE nivel = '$nivel' AND id_solicitacao = '$soli'";
	$stUpdateRead  = mysqli_query($conectar, $qrUpdateRead)or die (mysqli_errno());
   ...
   $st_soli  = mysqli_query($conectar, $qr_soli)or die (mysqli_errno()); 
    ...
    while ($as_soli = mysqli_fetch_assoc($st_soli)){
            ...
            if ($as_soli['status'] == 0) {
                $status = 'Aguardando...';
                $link_devo = '<a href="index.php?pagina=resposta&ms=' ;
            }
