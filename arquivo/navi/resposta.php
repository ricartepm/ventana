...
<?php if (isset ($_POST['add_resposta'])){
	...
	$sopros  =  mysqli_real_escape_string($conectar, $_POST['processo']);
	$user    =  mysqli_real_escape_string($conectar, $_POST['id_usuario']);  
	$userer  =  mysqli_real_escape_string($conectar, $_POST['usuario']);
...
<form action="" method="post" name="add_res" id="add_res">
    ...
    <input type="hidden" name="usuario" value="<?php echo $asms['id_usuario']; ?>";/>
    <input type="hidden" name="processo" value="<?php echo $asms['Nr_Processo']; ?>" />
