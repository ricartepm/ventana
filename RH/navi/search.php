<div id="header">
	...
	$soSta =  mysql_real_escape_string($_POST['stat']); 
        $soNr =  mysql_real_escape_string($_POST['nr']); 
   	$cadbusca = mysql_query("SELECT * FROM processo WHERE ID = '$sopro' AND status = '1' " );
     if (mysql_num_rows($cadbusca) == 0){
 ...
 try{
  ...
  foreach($resutado_inboxAdmin as $res_inboxAdmin){
        ...
        $sopro =  mysql_real_escape_string($proce_id); 
   	$cadbusca = mysql_query("SELECT * FROM caixa WHERE id_processo = '$sopro' " );
        if (mysql_num_rows($cadbusca) >= 1){       
          $solic = '<input type="submit" value="Solicitar" class="btn" />';
          }else{
	   $solic = '<input type="submit" value="Não arq..." class="btndis" disabled="disabled"   />' ;
		 }
                 
        $cadbuscas = mysql_query("SELECT * FROM processo WHERE ID = '$sopro' AND status = '1' " );
         if (mysql_num_rows($cadbuscas) == 0){
