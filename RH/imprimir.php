...
$RsImpre = mysql_query($query_limit_RsImpre) or die(mysql_error());
$row_RsImpre = mysql_fetch_assoc($RsImpre);

if (isset($_GET['totalRows_RsImpre'])) {
  $totalRows_RsImpre = $_GET['totalRows_RsImpre'];
} else {
  $all_RsImpre = mysql_query($query_RsImpre);
  $totalRows_RsImpre = mysql_num_rows($all_RsImpre);
}
...
do {
   ...
   ?>
	   ...
	   $qrproce = "SELECT * FROM processo WHERE ID = '$row_RsImpre[id_processo]'";
           $stproce = mysql_query($qrproce)or die (mysql_errno());
           $asproce = mysql_fetch_assoc($stproce);
...
} while ($row_RsImpre = mysql_fetch_assoc($RsImpre));
if($RsImpre_endRow != 0) {
