...
$query_RsGerador = "SELECT * FROM gerador_caixa ORDER BY id_caixa DESC LIMIT 10";
...
<form action="<?php echo $editFormAction; ?>" method="post" name="caixa" id="caixa">
    ...
    <tr>
      <th >Caixa:</th>
      <td><label for="caixa"></label>
        <select name="caixa" id="caixa">
          <?php
do {  
?>
          <option value="<?php echo $row_RsGerador['id_caixa']?>"><?php echo $row_RsGerador['id_caixa']?></option>
          <?php
} while ($row_RsGerador = mysqli_fetch_assoc($RsGerador));
  $rows = mysqli_num_rows($RsGerador);
  if($rows > 0) {
      mysqli_data_seek($RsGerador, 0);
	  $row_RsGerador = mysqli_fetch_assoc($RsGerador);
  }
?>
      </select></td>
    ...
    <tr >
      <th >Prateleira:</th>
      <td><input type="text" name="prateleira" value="" class="input" /></td>
    </tr>
    <tr >
        <th></th>
